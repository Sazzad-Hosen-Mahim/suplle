      ...
      >
        ...
        <div className="flex items-center space-x-3 group pr-2 py-1 rounded-full text-white">
          <div className="transform ease-in-out duration-300 mr-12">
            <Image src={sidebarlogo} width={70} height={30} alt="Logo" />
          </div>
        </div>
      </div>
        ...
        {ownerSeiderBarData.map((item) => {
          ...
          return (
            <Link
              key={item.id}
              href={item.url}
              className={`w-full text-white px-2 py-3 pl-8 flex items-center space-x-3 ${
        ...
        {ownerSeiderBarData.map((item) => {
          ...
          return (
            <Link
              key={item.id}
              href={item.url}
              className={`justify-end pr-5 text-white hover:text-purple-500 dark:hover:text-blue-500 w-full p-3 flex ${
                isActive ? "bg-green-700" : "hover:bg-green-300"
              }`}
            >
              <Icon size={22} color="white" />
